...
bool ArqmaMQ::proxy_check_auth(size_t conn_index, bool outgoing, const peer_info& peer,
    ...
    if (!cat_call.first) {
        AMQ_LOG(warn, "Invalid command '", command, "' sent by remote [", to_hex(peer.pubkey), "]/", peer_address(cmd));
        reply = "UNKNOWNCOMMAND";
    } else if (peer.auth_level < cat_call.first->access.auth) {
        AMQ_LOG(warn, "Access denied to ", command, " for peer [", to_hex(peer.pubkey), "]/", peer_address(cmd),
        ...
        AMQ_LOG(warn, "Received an invalid request for '", command, "' with no reply tag from remote [",
    ...
    try {
        ...
        AMQ_LOG(debug, "Couldn't send auth failure message ", reply, " to peer [", to_hex(peer.pubkey), "]/", peer_address(cmd), ": ", err.what());
    }

    return false;
}

void ArqmaMQ::set_active_sns(pubkey_set pubkeys) {
...
void ArqmaMQ::proxy_set_active_sns(std::string_view data) {
    proxy_set_active_sns(detail::deserialize_object<pubkey_set>(bt_deserialize<uintptr_t>(data)));
}
void ArqmaMQ::proxy_set_active_sns(pubkey_set pubkeys) {
    pubkey_set added, removed;
    for (auto it = pubkeys.begin(); it != pubkeys.end(); ) {
        auto& pk = *it;
        if (pk.size() != 32) {
            AMQ_LOG(warn, "Invalid private key of length ", pk.size(), " (", to_hex(pk), ") passed to set_active_sns");
            it = pubkeys.erase(it);
            continue;
        }
    ...
    if (added.empty() && active_service_nodes.size() == pubkeys.size()) {
        AMQ_LOG(debug, "set_active_sns(): new set of SNs is unchanged, skipping update");
        return;
    }
...
void ArqmaMQ::update_active_sns(pubkey_set added, pubkey_set removed) {
    AMQ_LOG(info, "uh, ", added.size());
    if (proxy_thread.joinable()) {
...
void ArqmaMQ::proxy_update_active_sns(bt_list_consumer data) {
...
void ArqmaMQ::proxy_update_active_sns(pubkey_set added, pubkey_set removed) {
    ...
    AMQ_LOG(info, "uh, ", added.size(), ", ", removed.size());
    for (auto it = removed.begin(); it != removed.end(); ) {
        const auto& pk = *it;
        if (pk.size() != 32) {
            AMQ_LOG(warn, "Invalid private key of length ", pk.size(), " (", to_hex(pk), ") passed to update_active_sns (removed)");
            it = removed.erase(it);
    ...
    for (auto it = added.begin(); it != added.end(); ) {
        const auto& pk = *it;
        if (pk.size() != 32) {
            AMQ_LOG(warn, "Invalid private key of length ", pk.size(), " (", to_hex(pk), ") passed to update_active_sns (added)");
            it = added.erase(it);
...
void ArqmaMQ::proxy_update_active_sns_clean(pubkey_set added, pubkey_set removed) {
    AMQ_LOG(debug, "Updating SN auth status with +", added.size(), "/-", removed.size(), " pubkeys");
    ...
    for (const auto& pk : removed) {
        ...
        for (auto it = range.first; it != range.second; ) {
            ...
            it = peers.erase(it);
            if (outgoing) {
                AMQ_LOG(debug, "Closing outgoing connection to ", c);
...
void ArqmaMQ::process_zap_requests() {
    for (std::vector<zmq::message_t> frames; recv_message_parts(zap_auth, frames, zmq::recv_flags::dontwait); frames.clear()) {
...
#endif
            AMQ_LOG(debug, "Processing ZAP authentication request");
        ...
        if (frames.size() < 6 || view(frames[0]) != "1.0") {
            AMQ_LOG(error, "Bad ZAP authentication request: version != 1.0 or invalid ZAP message parts");
            status_code = "500";
            ...
            if (bind_id >= bind.size()) {
                AMQ_LOG(error, "Bad ZAP authentication request: invalid auth domain '", auth_domain, "'");
                status_code = "400";
                ...
                AMQ_LOG(error, "Bad ZAP authentication request: invalid ",
                ...
                AMQ_LOG(error, "Bad ZAP authentication request: invalid request pubkey");
                status_code = "500";
                ...
                if (auth <= AuthLevel::denied || auth > AuthLevel::admin) {
                    AMQ_LOG(info, "Access denied for incoming ", view(frames[5]), (sn ? " service node" : " client"),
                    ...
                    AMQ_LOG(debug, "Accepted incoming ", view(frames[5]), (sn ? " service node" : " client"),
        ...
        AMQ_TRACE("ZAP request result: ", status_code, " ", status_text);
