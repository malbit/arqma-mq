...
bool worker_wait_for(ArqmaMQ& amq, zmq::socket_t& sock, std::vector<zmq::message_t>& parts, const std::string_view worker_id, const std::string_view expect)
{
  while (true)
  {
    amq.log(LogLevel::debug, __FILE__, __LINE__, "worker ", worker_id, " waiting for ", expect);
    parts.clear();
    recv_message_parts(sock, parts);
    if (parts.size() != 1)
    {
      amq.log(LogLevel::error, __FILE__, __LINE__, "Internal error: worker ", worker_id, " received invalid ", parts.size(), "-part control msg");
      continue;
    }
    ...
    {
#ifndef NDEBUG
      amq.log(LogLevel::trace, __FILE__, __LINE__, "Worker ", worker_id, " received waited-for ", expect, " command");
#endif
      return true;
    }
    else if (command == "QUIT"sv)
    {
      amq.log(LogLevel::debug, __FILE__, __LINE__, "Worker ", worker_id, " received QUIT command. Shutting down");
    ...
    {
      amq.log(LogLevel::error, __FILE__, __LINE__, "Internal error: worker ", worker_id, " received invalid command: `", command, "'");
    }
  }
}
...
void ArqmaMQ::worker_thread(unsigned int index, std::optional<std::string> tagged, std::function<void()> start) {
    std::string routing_id = (tagged ? "t" : "w") + std::to_string(index);
    std::string worker_id{tagged ? *tagged : routing_id};

    [[maybe_unused]] std::string thread_name = tagged.value_or("amq-" + routing_id);
    ...
    AMQ_LOG(debug, "New worker thread ", worker_id, " (", routing_id, ") started");
        ...
        try {
            if (run.is_batch_job) {
                ...
                if (run.batch_jobno >= 0) {
                    AMQ_TRACE("worker thread ", worker_id, " running batch ", batch, "#", run.batch_jobno);
                    batch->run_job(run.batch_jobno);
                } else if (run.batch_jobno == -1) {
                    AMQ_TRACE("worker thread ", worker_id, " running batch ", batch, " completion");
                    batch->job_completion();
                }
            } else if (run.is_injected) {
                auto& func = var::get<std::function<void()>>(run.to_run);
                AMQ_TRACE("worker thread ", worker_id, " invoking injected command ", run.command);
                func();
                func = nullptr;
            } else {
                ...
                AMQ_TRACE("Got incoming command from ", message.remote, "/", message.conn, message.conn.route.empty() ? " (outgoing)" : " (incoming)");
                ...
                AMQ_TRACE("worker thread ", worker_id, " invoking ", run.command, " callback with ", message.data.size(), " message parts");
                callback(message);
            }
        }
        catch (const bt_deserialize_invalid& e) {
            AMQ_LOG(warn, worker_id, " deserialization failed: ", e.what(), "; ignoring request");
        }
#ifndef BROKEN_APPLE_VARIANT
        catch (const std::bad_variant_access& e) {
            AMQ_LOG(warn, worker_id, " deserialization failed: found unexpected serialized type (", e.what(), "); ignoring request");
        }
#endif
        catch (const std::out_of_range& e) {
            AMQ_LOG(warn, worker_id, " deserialization failed: invalid data - required field missing (", e.what(), "); ignoring request");
        }
        catch (const std::exception& e) {
            AMQ_LOG(warn, worker_id, " caught exception when processing command: ", e.what());
        }
        catch (...) {
            AMQ_LOG(warn, worker_id, " caught non-standard exception when processing command");
        }
...
void ArqmaMQ::proxy_worker_message(std::vector<zmq::message_t>& parts) {
    ...
    if (parts.size() != 2) {
        AMQ_LOG(error, "Received send invalid ", parts.size(), "-part message");
        return;
    }
    auto route = view(parts[0]), cmd = view(parts[1]);
    AMQ_TRACE("worker message from ", route);
    ...
    if (!worker_id_str.empty() || (tagged_worker ? 0 == worker_id || worker_id > tagged_workers.size() : worker_id >= workers.size()))
    {
        AMQ_LOG(error, "Worker id '", route, "' is invalid, unable to process worker command");
        return;
    }

    auto& run = tagged_worker ? std::get<run_info>(tagged_workers[worker_id - 1]) : workers[worker_id];

    AMQ_TRACE("received ", cmd, " command from ", route);
    if (cmd == "RAN"sv) {
        AMQ_TRACE("Worker ", route, " finished ", run.is_batch_job ? "batch job" : run.command);
        if (run.is_batch_job) {
            if (tagged_worker)
            {
            ...
            if (run.batch_jobno == -1) {
                ...
                if (state == detail::BatchState::complete) {
                    if (thread == -1)
                    {
                      AMQ_TRACE("Completion job running directly in proxy");
                      try
                      {
                        batch->job_completion();
                      }
                      ...
                      {
                        AMQ_LOG(error, "proxy thread caught exception when processing in-proxy completion command: ", e.what());
                      } catch (...) {
                        AMQ_LOG(error, "proxy thread caught non-standard exception when processing in-proxy completion command");
                      }
                      clear_job = true;
                    }
                    else
                    {
                      auto& jobs = thread > 0
                                   ...
                                   : run.is_reply_job ? reply_jobs : batch_jobs;
        ...
        if (max_workers == 0) { // Shutting down
            AMQ_TRACE("Telling worker ", route, " to quit");
        ...
        AMQ_LOG(debug, "Worker ", route, " exited normally");
    } else {
        AMQ_LOG(error, "Worker ", route, " sent unknown control message: `", cmd, "'");
    }
}
...
void ArqmaMQ::proxy_to_worker(size_t conn_index, std::vector<zmq::message_t>& parts) {
    ...
    if (outgoing) {
        ...
        if (it == peers.end()) {
            AMQ_LOG(warn, "Internal error: connection index ", conn_index, " not found");
            return;
        }
        peer = &it->second;
    } else {
    ...
    if (category.active_threads >= category.reserved_threads && active_workers() >= general_workers) {
        ...
        if (category.max_queue >= 0 && category.queued >= category.max_queue) {
            AMQ_LOG(warn, "No space to queue incoming command ", command, "; already have ", category.queued,
                    "commands queued in that category (max ", category.max_queue, "); dropping message");
            return;
        }

        AMQ_LOG(debug, "No available free workers, queuing ", command, " for later");
    ...
    if (cat_call.second->second /*is_request*/ && data_parts.empty()) {
        AMQ_LOG(warn, "Received an invalid request command with no reply tag; dropping message");
        return;
    }
    ...
    auto& run = get_idle_worker();
    {
        ...
        run.load(&category, std::move(command), std::move(c), std::move(access), peer_address(parts[command_part_index]), std::move(data_parts), cat_call.second);
    }

    if (outgoing)
        peer->activity(); // outgoing connection activity, pump the activity timer

    AMQ_TRACE("Forwarding incoming ", run.command, " from ", run.conn, " @ ", peer_address(parts[command_part_index]),
...
void ArqmaMQ::inject_task(const std::string& category, std::string command, std::string remote, std::function<void()> callback)
{
  if (!callback) return;
  auto it = categories.find(category);
  if (it == categories.end())
    throw std::out_of_range{"Invalid category `" + category + "': category does not exist"};
  detail::send_control(get_control_socket(), "INJECT", bt_serialize(detail::serialize_object(injected_task{it->second, std::move(command), std::move(remote), std::move(callback)})));
}

void ArqmaMQ::proxy_inject_task(injected_task task)
{
  ...
  if (category.active_threads >= category.reserved_threads && active_workers() >= general_workers)
  {
    if (category.max_queue >= 0 && category.queued >= category.max_queue)
    {
      AMQ_LOG(warn, "No space to queue injected task ", task.command, "; already have ", category.queued, " commands queued in that category (max ", category.max_queue, "); dropping task");
      return;
    }
    AMQ_LOG(debug, "No available free workers for injected task ", task.command, "; queuing for later");
  ...
  auto& run = get_idle_worker();
  AMQ_TRACE("Forwarding incoming injected tast ", task.command, " from ", task.remote, " to worker ", run.worker_routing_id);
