...
void ArqmaMQ::set_zmq_context_option(zmq::ctxopt option, int value) {
    return context.set(option, value);
}
        ...
        sn_lookup{std::move(lookup)}, log_lvl{level}, logger{std::move(logger)}
{
    ...
    AMQ_TRACE("Constructing ArqmaMQ, id=", object_id, ", this=", this);
    ...
    if (pubkey.empty() != privkey.empty()) {
        ...
        AMQ_LOG(debug, "generating x25519 keypair for remote-only ArqmaMQ instance");
...
void ArqmaMQ::start() {
    ...
    AMQ_LOG(info, "Initializing ArqmaMQ ", bind.empty() ? "remote-only" : "listener", " with pubkey ", to_hex(pubkey));
    ...
    if (MAX_SOCKETS > 1 && MAX_SOCKETS <= zmq_socket_limit)
      ...
      AMQ_LOG(error, "Not applying ArqmaMQ::MAX_SOCKETS setting: ", MAX_SOCKETS, " must be in [1, ", zmq_socket_limit, "]");
    ...
    AMQ_LOG(debug, "Waiting for proxy thread to get ready...");
    auto &control = get_control_socket();
    detail::send_control(control, "START");
    AMQ_TRACE("Sent START command");
    ...
    AMQ_LOG(debug, "Proxy thread is ready");
}
...
std::pair<ArqmaMQ::category*, const std::pair<ArqmaMQ::CommandCallback, bool>*> ArqmaMQ::get_command(std::string& command) {
    if (command.size() > MAX_CATEGORY_LENGTH + 1 + MAX_COMMAND_LENGTH) {
        AMQ_LOG(warn, "Invalid command '", command, "': command too long");
        return {};
    }
    ...
    if (dot == 0 || dot == std::string::npos) {
        AMQ_LOG(warn, "Invalid command '", command, "': expected <category>.<command>");
        return {};
    }
    ...
    if (catit == categories.end()) {
        AMQ_LOG(warn, "Invalid command category '", catname, "'");
        return {};
    }
    ...
    if (callback_it == category.commands.end()) {
        AMQ_LOG(warn, "Invalid command '", command, "'");
        return {};
    }
...
ArqmaMQ::run_info& ArqmaMQ::run_info::load(category* cat_, std::string command_, ConnectionID conn_, Access access_, std::string remote_,
...
ArqmaMQ::run_info& ArqmaMQ::run_info::load(category* cat_, std::string command_, std::string remote_, std::function<void()> callback)
{
  reset();
  is_injected = true;
  cat = cat_;
...
ArqmaMQ::run_info& ArqmaMQ::run_info::load(batch_job&& bj, bool reply_job, int tagged_thread) {
    reset();
    is_batch_job = true;
...
ArqmaMQ::~ArqmaMQ() {
    ...
    AMQ_LOG(info, "ArqmaMQ shutting down proxy thread");
    detail::send_control(get_control_socket(), "QUIT");
    proxy_thread.join();
    AMQ_LOG(info, "ArqmaMQ proxy thread has stopped");
}
