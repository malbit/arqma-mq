...
void ArqmaMQ::proxy_batch(detail::Batch* batch) {
    ...
    AMQ_TRACE("proxy queuing batch job with ", jobs, " jobs", tagged_threads ? " (job uses tagged thread(s))" : "");
    if (!tagged_threads)
    {
    ...
    else
    {
      ...
      for (size_t i = 0; i < jobs; i++)
      {
        auto& jobs = threads[i] > 0 ? std::get<std::queue<batch_job>>(tagged_workers[threads[i] - 1]) : batch_jobs;
...
void ArqmaMQ::job(std::function<void()> f, std::optional<TaggedThreadID> thread) {
...
void ArqmaMQ::_queue_timer_job(int timer_id) {
    ...
    if (it == timer_jobs.end()) {
        AMQ_LOG(warn, "Could not find timer job ", timer_id);
        return;
    }
    auto& [func, squelch, running, thread] = it->second;
    if (squelch && running) {
        AMQ_LOG(debug, "Not running timer job ", timer_id, " because a job for that timer is still running");
        return;
    }
    ...
    {
      try { func(); }
      catch (const std::exception &e) { AMQ_LOG(warn, "timer job ", timer_id, " raised an exception: ", e.what()); }
      catch (...) { AMQ_LOG(warn, "timer job ", timer_id, " raised a non-std exception"); }
      return;
    }
    ...
    if (squelch) {
        ...
        }, ArqmaMQ::run_in_proxy);
    }
    batches.insert(b);
    AMQ_TRACE("b: ", b->size().first, ", ", b->size().second, "; thread = ", thread);
    assert(b->size() == std::make_pair(size_t{1}, thread > 0));
    auto& queue = thread > 0 ? std::get<std::queue<batch_job>>(tagged_workers[thread - 1]) : batch_jobs;
...
TaggedThreadID ArqmaMQ::add_tagged_thread(std::string name, std::function<void()> start)
{
  if (proxy_thread.joinable())
  ...
  busy = false;
  run.worker_id = tagged_workers.size();
  run.worker_routing_id = "t" + std::to_string(run.worker_id);
  AMQ_TRACE("Created new tagged thread ", name, " with routing id ", run.worker_routing_id);

  run.worker_thread = std::thread{&ArqmaMQ::worker_thread, this, run.worker_id, name, std::move(start)};
