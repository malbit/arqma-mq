...
void ArqmaMQ::setup_external_socket(zmq::socket_t& socket) {
...
void ArqmaMQ::setup_outgoing_socket(zmq::socket_t& socket, std::string_view remote_pubkey, bool use_ephemeral_routing_id) {
...
ConnectionID ArqmaMQ::connect_remote(std::string_view remote, ConnectSuccess on_connect, ConnectFailure on_failure, AuthLevel auth_level, std::chrono::milliseconds timeout)
{
  return connect_remote(address{remote}, std::move(on_connect), std::move(on_failure), auth_level, connect_option::timeout{timeout});
}

ConnectionID ArqmaMQ::connect_remote(std::string_view remote, ConnectSuccess on_connect, ConnectFailure on_failure, std::string_view pubkey, AuthLevel auth_level, std::chrono::milliseconds timeout)
{
  return connect_remote(address{remote}.set_pubkey(pubkey), std::move(on_connect), std::move(on_failure), auth_level, connect_option::timeout{timeout});
}
...
ArqmaMQ::proxy_connect_sn(std::string_view remote, std::string_view connect_hint, bool optional, bool incoming_only, bool outgoing_only, bool use_ephemeral_routing_id, std::chrono::milliseconds keep_alive) {
    ...
    if (peer) {
        AMQ_TRACE("proxy asked to connect to ", to_hex(remote), "; reusing existing connection");
        if (peer->route.empty() /* == outgoing*/) {
            if (peer->idle_expiry < keep_alive) {
                AMQ_LOG(debug, "updating existing outgoing peer connection idle expiry time from ",
        ...
        AMQ_LOG(debug, "proxy asked for optional or incoming connection, but no appropriate connection exists so aborting connection attempt");
        return {nullptr, ""s};
    }

    // No connection so establish a new one
    AMQ_LOG(debug, "proxy establishing new outbound connection to ", to_hex(remote));
    std::string addr;
    ...
    if (to_self) {
        ...
        if (addr.empty())
            ...
            AMQ_LOG(debug, "using connection hint ", connect_hint);

        if (addr.empty()) {
            AMQ_LOG(error, "peer lookup failed for ", to_hex(remote));
            return {nullptr, ""s};
        }
    }

    AMQ_LOG(debug, to_hex(pubkey), " (me) connecting to ", addr, " to reach ", to_hex(remote));
    zmq::socket_t socket{context, zmq::socket_type::dealer};
    setup_outgoing_socket(socket, remote, use_ephemeral_routing_id);
    try {
      socket.connect(addr);
    } catch (const zmq::error_t& e) {
      AMQ_LOG(error, "Outgoing connection to ", addr, " failed: ", e.what());
      return {nullptr, ""s};
    }
    peer_info p{};
    p.service_node = true;
...
void ArqmaMQ::proxy_close_connection(size_t index, std::chrono::milliseconds linger) {
    ...
    AMQ_LOG(debug, "Closing conn index ", index);
    ...
    for (auto it = peers.begin(); it != peers.end(); ) {
        ...
        if (info.outgoing()) {
            ...
            if (idle > info.idle_expiry) {
              AMQ_LOG(debug, "Closing outgoing connection to ", it->first, ": idle time (",
              ...
              AMQ_LOG(trace, "Not closing ", it->first, ": ", std::chrono::duration_cast<std::chrono::milliseconds>(idle).count(), "ms <= ", info.idle_expiry.count(), "ms");
              ++it;
              continue;
            }
        } else {
...
void ArqmaMQ::proxy_conn_cleanup() {
    AMQ_TRACE("starting proxy connections cleanup");
    ...
    AMQ_TRACE("closing idle connections");
    ...
    AMQ_TRACE("Timing out pending outgoing connections");
    ...
    AMQ_TRACE("Timing out pending requests");
    // Remove any expired pending requests and schedule their callback with a failure
    for (auto it = pending_requests.begin(); it != pending_requests.end(); ) {
        auto& callback = it->second;
        if (callback.first < now) {
            AMQ_LOG(debug, "pending request ", to_hex(it->first), " expired, invoking callback with failure status and removing");
    ...
    AMQ_TRACE("done proxy connections cleanup");
};
...
void ArqmaMQ::proxy_connect_remote(bt_dict_consumer data) {
    ...
    AMQ_LOG(debug, "Establishing remote connection to ", remote, remote_pubkey.empty() ? " (NULL auth)" : " via CURVE expecting pubkey " + to_hex(remote_pubkey));
    ...
    pollitems_stale = true;
    AMQ_LOG(debug, "Opened new zmq socket to ", remote, ", conn_id ", conn_id, "; sending HI");
...
void ArqmaMQ::proxy_disconnect(ConnectionID conn, std::chrono::milliseconds linger) {
    AMQ_TRACE("Disconnecting outgoing connection to ", conn);
    auto pr = peers.equal_range(conn);
    for (auto it = pr.first; it != pr.second; ++it) {
        auto& peer = it->second;
        if (peer.outgoing()) {
            AMQ_LOG(debug, "Closing outgoing connection to ", conn);
            proxy_close_connection(peer.conn_index, linger);
            return;
        }
    }
    AMQ_LOG(warn, "Failed to disconnect ", conn, ": no such outgoing connection");
}
